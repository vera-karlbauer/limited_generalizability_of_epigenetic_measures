---
title: "Evaluating epigenetic clocks and scores: epigenetic score performance"
...
- Combine lineplots for each epigenetic score with/without celltype correction into one plot (Supplementary Figure 3).
- Select exemplary line plots (for CRP Wielscher, DNAmTL) and export as Figure 3.
- Export results as Supplementary Table 3.
...
results_kin_nocelltypes <- data.frame(matrix(nrow = 0, ncol = 7))
colnames(results_kin_nocelltypes) <- c("sample_size",
                                       "coef_pheno", "p_pheno", 
...
for(score in unique_scores$episcore){
  ...
  current_data = select(episcores_kin, score, pheno, tissue, child_id) %>%
    rename("score" = score,
      "pheno" = pheno) %>%
    filter(!is.na(score) & !is.na(pheno))
  # fit model
...
# create empty results object
results_kin_celltypes <- data.frame(matrix(nrow = 0, ncol = 7))
colnames(results_kin_celltypes) <- c("sample_size",
                                     "coef_pheno", "p_pheno", 
                                         "coef_tissue", "p_tissue", 
                                         "coef_interaction", 
                                         "p_interaction")
...
## Join all values, FDR-correct, round, export as table (Supplementary Table 3)
...
results_episcore_performance_kin <- full_join(results_kin_nocelltypes, temp_celltype,
                                              join_by("episcore" == "episcore_celltype_adjusted",
                                                      "phenotype" == "phenotype_celltype_adjusted",
                                                      "sample_size" == "sample_size_celltype_adjusted"))
...
# assign measures to scenario 1 (= equal performance across tissues after cell type correction) or scenario 2 (= unequal performance or n.s. association with phenotype across tissues after cell type correction)
results_episcore_performance_full <- results_episcore_performance_full %>%
  mutate(performance_scenario = case_when(
    # if interaction effect is n.s. after FDR and cell type correction
    p_interaction_fdr_celltype_adjusted >= 0.05
    # AND main phenotype effect is significant after cell type correction
    & p_pheno_celltype_adjusted < 0.05
    # assign to scenario 1
    ~ "Scenario 1",
    # if interaction effect is significant after FDR and cell type correction
    p_interaction_fdr_celltype_adjusted < 0.05
    # or if interaction effect is n.s. after FDR and cell type correction, but main phenotype effect is also n.s. after cell type correction
    | (p_interaction_fdr_celltype_adjusted >= 0.05 & p_pheno_celltype_adjusted >= 0.05)
    # assign to scenario 2
    ~ "Scenario 2",
    # otherwise flag
    TRUE ~ "no scenario assigned"
  ))
...
write_xlsx(results_episcore_performance_full, path = "05_tables_for_publication/supptable_3_episcore_performance.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
for(score in unique_scores$episcore){
  ...
  print(annotate_figure(combined,
                top = text_grob("unadjusted                                             cell type adjusted",
                                size = 14, face = "bold")))
  # export
  dev.off()
}
...
### Combine all line plots and export (= Supplementary Figure 3)

```{r}
## arrange & export all plots
png(filename = "./04_figures/supplementary_figure_3_combined_lineplot_all_scores.png", 
...
annotate_figure(combined_lineplot,
                top = text_grob("         unadjusted            cell type adjusted           unadjusted            cell type adjusted",
                                                     size = 14, face = "bold"), 
                fig.lab.pos = "top")
...
## Plot selected results only (Figure 3 for publication)
...
png(filename = "./04_figures/figure_3_lineplot_episcore_performance.png", 
