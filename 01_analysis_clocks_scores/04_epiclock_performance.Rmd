---
title: "Evaluating epigenetic clocks and scores: epigenetic clock performance"
...
- Combine lineplots for each epigenetic clock with/without celltype correction into one plot (Supplementary Figure 2).
- Select exemplary line plots (for GrimAge2, PedBE) and export as Figure 2.
- Export results as Supplementary Table 2.
...
## Join all values, FDR-correct, assign scenario, round values, export as table (Supplementary Table 2)
...
results_epiclock_performance_full <- results_epiclock_performance_full %>%
  mutate(performance_scenario = case_when(
    ...
    & p_chrono_age_celltype_adjusted < 0.05
    # assign to scenario 1
    ~ "Scenario 1",
    # if interaction effect is significant after FDR and cell type correction
    p_interaction_fdr_celltype_adjusted < 0.05
    # or if interaction effect n.s. after FDR and cell type correction, but main phenotype effect is n.s. after cell type correction
    | (p_interaction_fdr_celltype_adjusted >= 0.05 & p_chrono_age_celltype_adjusted >= 0.05)
...
# export
write_xlsx(results_epiclock_performance_full, path = "05_tables_for_publication/supptable_2_epiclock_performance.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
for(clock in unique_clocks$epiclock){
  ...
  p = ggplot(current_data, 
    ...
    scale_color_brewer(palette = "Set1", name = "Tissue", breaks = c("blood", "saliva")) +
    scale_shape_manual(values = c(16, 17), name = "Tissue", breaks = c("blood", "saliva")) +
...
for(clock in unique_clocks$epiclock_resid){
  ...
  p = ggplot(current_data, 
    ...
    scale_color_manual(values=c("#EB5355", "#699ECA"), 
                              name = "Tissue", breaks = c("blood", "saliva")) +
    scale_shape_manual(values = c(16, 17), name = "Tissue", breaks = c("blood", "saliva")) +
    ggtitle(rename_clocks(clock)) +
    xlab("Chronological age") + 
...
### Combine all line plots and export (= Supplementary Figure 2)

```{r}
## arrange & export all plots
png(filename = "./04_figures/supplementary_figure_2_combined_lineplot_all_clocks.png", 
...
## Plot select results only (Figure 2 for publication)
...
combined = ggarrange(lineplot_nocell_epiage_grimage2, lineplot_cell_epiage_grimage2_resid,
  lineplot_nocell_epiage_pedbe, lineplot_cell_epiage_pedbe_resid,
                        ...
                        legend = "bottom",
  labels = c("A","B","C","D"))
png(filename = "./04_figures/figure_2_lineplot_epiclock_performance.png", width = 20, height = 14, res = 700, units = "cm")
  combined
dev.off()
