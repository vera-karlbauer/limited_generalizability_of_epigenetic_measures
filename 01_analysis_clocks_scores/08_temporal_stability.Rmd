---
title: "Evaluating epigenetic clocks and scores: temporal stability"
...
- Analyze correlations with and without cell type adjustment.
- For all correlations, also calculate corresponding ICC values (ICC2/random effects).
- Plot results as corrplots (Figure 5).
- Export results as Supplementary Tables 6-9.
...
## Correlations between timepoints (per tissue) with and without cell type adjustment
...
### Arrange and export final plot (Figure 5A) 

```{r}
# initialize graphics device
tiff(file = "04_figures/figure_5a_corrplot_all_epiage_measures_across_time.png", height = 10, width = 5.5, units = 'in', res = 700)
par(mfrow=c(2,2),
    oma = c(0,3,4,0))
...
mtext("without cell type adjustment", side=3, line=4, cex = 1)
...
corrplot(corr = corrmat_cell_blood,
         ...
         na.label = "NA",
         mar = c(0,0,0,1))
mtext("with cell type adjustment", side=3, line=4, cex = 1)
...
# for each epiage measure, calculate ICC2 (random effects) with and without cell type adjustment
...
#### Export blood longitudinal clock correlations as table (Supplementary Table 6)
...
# export
write_xlsx(longitudinal_epiclocks_blood, path = "05_tables_for_publication/supptable_6_longitudinal_epiclocks_blood.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
#### Export saliva longitudinal clock correlations as table (Supplementary Table 7)
...
# export
write_xlsx(longitudinal_epiclocks_saliva, path = "05_tables_for_publication/supptable_7_longitudinal_epiclocks_saliva.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
### Combined plot: arrange final plot (Figure 5B)
```{r}
# initialize graphics device
tiff(file = "04_figures/figure_5b_corrplot_all_episcores_across_time.png", height = 10, width = 5, units = 'in', res = 700)
par(mfrow=c(2,2),
    oma = c(0,0,4,0))
# Plot 1: blood without cell types
...
# for each episcore, calculate ICC2 (random effects) with and without cell type adjustment
...
#### Export blood longitudinal correlations as table (Supplementary Table 8)
...
# export
write_xlsx(correlations_episcores, path = "05_tables_for_publication/supptable_8_longitudinal_episcores_blood.xlsx", col_names = TRUE, format_headers = TRUE)
...
#### Export saliva longitudinal correlations as table (Supplementary Table 9)
...
# export
write_xlsx(correlations_episcores, path = "05_tables_for_publication/supptable_9_longitudinal_episcores_saliva.xlsx", col_names = TRUE, format_headers = TRUE)
