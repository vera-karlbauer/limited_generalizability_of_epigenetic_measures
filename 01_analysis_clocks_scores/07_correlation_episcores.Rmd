---
title: "Evaluating epigenetic clocks and scores: epigenetic score cross-tissue correlations"
...
- Calculate correlations separately for unadjusted and celltype-adjusted scores.
- For all correlations, also calculate corresponding ICC values (ICC2/random effects).
- Plot results as corrplots (Figure 4 C-D).
- Export results as Supplementary Table 5.
...
### Step 3: Subset data (T0 only, scores only, with vs. without cell type adjustment)
...
### Format data without cell type adjustment
...
### Format data with cell type adjustment
...
### Create combined correlation plot (Figure 4C & 4D)

```{r}
# initialize graphics device
tiff(file = "04_figures/figure_4cd_corrplot_all_episcores_cross_tissue.png", height = 6.7, width = 5, units = 'in', res = 700)
par(mfrow=c(1,2),
    mar = c(1,1,0,0),
    oma = c(0,0,3,0))
...
corrplot(corr = corrmat_nocell,
         ...
         na.label = "NA",
         mar = c(0,0.2,0,1))
mtext("C", side = 3, line = 1, at = -2.9, cex = 1.2, font = 2)
mtext("Scores (unadjusted)", side = 3, line = 1, at = -0.5, cex = 1, font = 2)
...
corrplot(corr = corrmat_cell,
         ...
         na.label = "NA", 
         mar = c(0,0.2,0,1))
mtext("D", side = 3, line = 1, at = -3, cex = 1.2, font = 2)
mtext("Scores (cell type adjusted)", side = 3, line = 1, at = -0.4, cex = 1, font = 2)
...
## Export (Supplementary Table 5)

```{r}
# export
write_xlsx(correlations_episcores, path = "05_tables_for_publication/supptable_5_results_correlations_episcores.xlsx", col_names = TRUE, format_headers = TRUE)
```
