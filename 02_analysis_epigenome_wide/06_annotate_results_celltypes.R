...
### Purpose: combine results for each chunk of data, add annotation, export
...
# import first chunk
filename <- "03_results/epigenome_wide_crosstissue_celltype_1.Rdata"
load(filename)
results_celltype_combined <- resultsobj
...
for(chunk in 2:chunks){
  ...
  load(filename)
  results_celltype_combined <- rbind(results_celltype_combined, resultsobj)
}

### FDR-correct p-values
results_celltype_combined$p_fdr <- p.adjust(results_celltype_combined$p, method = "fdr")

### Append annotation (hg19)
results_celltype_combined <- left_join(results_celltype_combined, annotation_hg19, join_by("cpg_name" == "probeID"))
results_celltype_combined <- left_join(results_celltype_combined, manifest_hg38, join_by("cpg_name" == "IlmnID"))
...
### Export
save(results_celltype_combined, file = "03_results/results_celltype_combined_annotated.Rdata")
